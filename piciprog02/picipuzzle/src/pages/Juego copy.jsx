...
function Juego(initialStateJuego) {
  ...
  function dragEnd(e) {
    ...
    if (isAdjacent) {
      ...
      //actualPieza = { [piezaB };
      //piezaVacia = { [piezaA };
      //atualPieza = document.getElementById(`${imagenVaciaId}`);
  ...
  function empezarJuego(actualImgNImg, actualImgNumOrdImg) {
    ...
    desordenarArray(imgOrderRamdon);
    console.log(imgOrderRamdon);
    debugger;
    ...
    //  for (let arr = [...imgOrderRamdon];let arrCopia = [];let i = 0; i < imgOrderRamdon.length; i++) {
    //    let random = arr[Math.floor(Math.random() * arr.length)];
    //        console.log("Random:", random);
    // arrCopia.push(random);
    // if (random===0){arr.splice(arr.length, 1);:arr.splice(arr.length, 1);}
    ...
    // let ultimaPieza = { [arrayPiezas[i] };    // ctx.drawImage(ultimaPieza, posW, posH, state.ancho, state.alto);
    // const dataURL = piezaCanvas.toDataURL();
    // piezaFuera = document.querySelector('.img-pieza-fuera');
    // piezaFuera.src = dataURL;
    // piezaFuera.id = 'vacia';
...
//       piza.src = '../images/' + imgOrderRamdon[i] + '.jpg';
